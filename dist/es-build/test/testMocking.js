...
import { buildSchemaFromTypeDefinitions, addResolveFunctionsToSchema, makeExecutableSchema, } from '../schemaGenerator';
import 'mocha';
describe('Mock', () => {
    const shorthand = `
  ...
  `;
    ...
    it('throws an error if the property "schema" on the first argument is not of type GraphQLSchema', () => {
        expect(() => addMockFunctionsToSchema({ schema: {} }))
    ...
    it('mockServer is able to preserveResolvers of a prebuilt schema', () => {
      ...
      returnBirdsAndBees {
        ...
        return mockServer(jsSchema, mockMap, true).query(testQuery).then((res) => {
            expect(res.data.returnInt).to.equal(12345);
    ...
    it('lets you use mockServer with prebuilt schema', () => {
      ...
      returnBirdsAndBees {
        ...
        return mockServer(jsSchema, mockMap).query(testQuery).then((res) => {
            expect(res.data.returnInt).to.equal(12345);
            expect(res.data.returnFloat).to.be.a('number').within(-1000, 1000);
            expect(res.data.returnBoolean).to.be.a('boolean');
            expect(res.data.returnString).to.be.a('string');
    ...
    it('does not mask resolve functions if you tell it not to', () => {
        ...
        const mockMap = {
            RootQuery: () => ({
                ...
                returnString: (root, args) => Promise.resolve('foo'),
            }),
        };
        const resolvers = {
            RootQuery: {
                returnInt: () => 5,
                returnString: () => Promise.resolve('bar'),
            },
        };
        addResolveFunctionsToSchema(jsSchema, resolvers);
        addMockFunctionsToSchema({
    ...
    `;
        ...
        const mockMap = {
            ...
            Thread: () => ({
                name: 'Lorem Ipsum',
                posts: (o, a) => (new MockList(ITEMS_PER_PAGE * a['num'], (oi, ai) => ({ id: ai['num'] }))),
            }),
            Post: () => ({
                id: '41ae7bd',
                text: 'superlongpost',
