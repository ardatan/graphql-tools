...
export function stitchSchemas({
  ...
  schemas.forEach(schemaLikeObject => {
    if (isSchema(schemaLikeObject) || isSubschemaConfig(schemaLikeObject)) {
      ...
      const extensionsDocument = extractExtensionDefinitions(parsedSchemaDocument);
  ...
  let stitchedSchema = new GraphQLSchema({
    ...
    directives: directives.length ? directives.map(directive => cloneDirective(directive)) : undefined,
  });
  ...
  if (schemaDirectives != null) {
    SchemaDirectiveVisitor.visitSchemaDirectives(stitchedSchema, schemaDirectives);
  }
