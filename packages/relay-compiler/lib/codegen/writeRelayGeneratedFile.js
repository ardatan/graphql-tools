...
function writeRelayGeneratedFile(
  ...
  var moduleName = (
    languagePlugin === null || languagePlugin === void 0 ? void 0 : languagePlugin.getModuleName
  )
    ? languagePlugin.getModuleName(operationName)
    : operationName + '.graphql';
  var filename = moduleName + '.' + extension;
  var queryParametersFilename =
    generatedNode.kind === 'Request'
      ? ''.concat(generatedNode.params.name, '$Parameters.').concat(extension)
      : null;
  ...
  return Promise.resolve().then(
    /*#__PURE__*/ _asyncToGenerator(function* () {
      ...
      if (generatedNode.kind === RelayConcreteNode.REQUEST) {
        !(docText != null)
          ? process.env.NODE_ENV !== 'production'
            ? invariant(false, 'writeRelayGeneratedFile: Expected `text` for operations to be set.')
            : invariant(false)
          : void 0;
        var _generatedNode$params = generatedNode.params.metadata,
          ...
          nextMetadata = (0, _objectWithoutPropertiesLoose2['default'])(_generatedNode$params, [
            'isRefetchableQuery',
            'derivedFrom',
          ]);
        ...
        generatedNode = (0, _objectSpread2['default'])(
          (0, _objectSpread2['default'])({}, generatedNode),
          {},
          {
            // $FlowFixMe[incompatible-type]
      ...
      if (
        ...
        ((_generatedNode$metada = generatedNode.metadata) === null ||
        _generatedNode$metada === void 0
          ? void 0
          : _generatedNode$metada.derivedFrom) != null
      ) {
        var _generatedNode$metada2 = generatedNode.metadata,
          _ignored3 = _generatedNode$metada2.derivedFrom,
          metadata = (0, _objectWithoutPropertiesLoose2['default'])(_generatedNode$metada2, [
            'derivedFrom',
          ]); // $FlowFixMe[incompatible-type]
      ...
      if (
        ...
        generatedNode.kind === RelayConcreteNode.REQUEST && // $FlowFixMe[incompatible-type]
        generatedNode.params.operationKind === 'query'
      ) {
        writeQueryParameters(
          codegenDir,
          queryParametersFilename,
          moduleName, // $FlowFixMe[incompatible-call]
