      ...
      try {
        var _loop = function _loop() {
          var _step2$value = _step2.value,
            ...
            val = _step2$value[1];
          entryPromises.push(
            f(val, key, _this).then(function (resultVal) {
  ...
  _proto.get = function get(key) {
    !this.has(key)
      ? process.env.NODE_ENV !== 'production'
        ? invariant(false, 'StrictMap: trying to read non-existent key `%s`.', String(key))
        : invariant(false)
      : void 0; // $FlowFixMe[incompatible-return] - we checked the key exists
