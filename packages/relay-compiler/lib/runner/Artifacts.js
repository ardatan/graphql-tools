...
function serializeState(state) {
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done; ) {
      ...
      json.push([
        name,
        Array.from(artifacts).map(function (filename) {
          var _state$metadata$get;

          return [
            filename,
            (_state$metadata$get = state.metadata.get(filename)) !== null &&
            _state$metadata$get !== void 0
              ? _state$metadata$get
              : '',
          ];
        }),
      ...
      try {
        for (_iterator9.s(); !(_step9 = _iterator9.n()).done; ) {
          ...
          nextState.metadata.set(
            filename,
            (_generatedArtifacts$m = generatedArtifacts.metadata.get(filename)) !== null &&
              _generatedArtifacts$m !== void 0
              ? _generatedArtifacts$m
              : '',
          );
        }
      } catch (err) {
...
function producedFiles(dirs, artifactsMetadata) {
  ...
  dirs.forEach(function (_ref3) {
    ...
    []
      .concat(
        (0, _toConsumableArray2['default'])(updated),
        (0, _toConsumableArray2['default'])(created),
      )
    ...
    unchanged.forEach(function (filename) {
      ...
      result.set(
        name,
        sha1hex !== null && sha1hex !== void 0 ? sha1hex : sha1(nullthrows(dir.read(filename))),
      );
    });
  });
  return result;
