...
function visitTransformedDirective(node, state) {
  if (state.rootClientSelection) {
    throwError(
      '@'.concat(NODEKIND_DIRECTIVE_MAP[node.kind]),
      node.loc,
  ...
  if (
    node.selections.every(function (sel) {
    ...
    throwError(
      ...
      clientExtension && clientExtension.kind === 'ClientExtension'
        ? (_clientExtension$sele = clientExtension.selections[0]) === null ||
          _clientExtension$sele === void 0
          ? void 0
          : _clientExtension$sele.loc
        : null,
    );
  } // $FlowFixMe[incompatible-use]
...
function throwError(directiveName, directiveLoc, clientExtensionLoc) {
  throw createUserError(
    'Unexpected directive: '.concat(directiveName, '. ') +
