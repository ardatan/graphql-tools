  ...
  if (definition == null) {
    ...
    if (!isLegacyFatInterface) {
      throw createUserError(
        "Unknown field '".concat(node.name, "' on type ") +
    ...
    for (_iterator.s(); !(_step = _iterator.n()).done; ) {
      ...
      if (arg.defaultValue == null && schema.isNonNull(arg.type) && !nodeArgsSet.has(arg.name)) {
        throw createUserError(
          "Required argument '"
            .concat(arg.name, ': ')
            .concat(schema.getTypeString(arg.type), "' ") +
            "is missing on '".concat(node.name, "' in '").concat(rootNode.name, "'."),
