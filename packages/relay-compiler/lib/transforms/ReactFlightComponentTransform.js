...
function visitInlineFragment(fragment, state) {
  ...
  return this.traverse(fragment, {
    parentType:
      (_fragment$typeConditi = fragment.typeCondition) !== null && _fragment$typeConditi !== void 0
...
function visitScalarField(field, state) {
  ...
  if (clientField == null) {
    throw createCompilerError(
      "Definition not found for field '"
        .concat(schema.getTypeString(state.parentType), '.')
        .concat(field.name, "'"),
      [field.loc],
    );
  }
  ...
  var componentNameArg =
    componentDirective === null || componentDirective === void 0
      ? void 0
      : componentDirective.args.find(function (arg) {
  ...
  if (flightField == null) {
    throw createUserError(
      "Invalid Flight field, expected the parent type '".concat(
        schema.getTypeString(state.parentType),
        "' ",
      ) +
