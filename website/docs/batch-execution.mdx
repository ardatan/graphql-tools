...
query {
  _0_field1: field1
  _0_field3: field3(input: "A")
  _1_tricky: field2
  _1_field3: field3(input: "B")
}
```
...
> _Note:_ an alternative batching pattern transmits multiple operations [as an array](https://www.apollographql.com/blog/apollo-client/performance/batching-client-graphql-queries/#5853), and requires services to be specially configured for array inputs. GraphQL Tools batching is basic GraphQL, and is therefore compatible with all services.
...
const [first, second] = await Promise.all([
  myBatchExec({ document: parse('{ a:field1 b:field2 }') }),
  myBatchExec({ document: parse('{ field3 }') }),
]);
...
When using a batch executor, remember that multiple calls must be performed synchronously and all results awaited as one. Awaiting the results of each batching call individually will provide no benefit.

## Merging algorithm

Batch merging uses several transformations to build a request:

1. Replace top-level fragment spreads with inline fragments.
2. Add unique aliases to all top-level query fields.
...
3. Redistribute pathed errors among their corresponding requests.
