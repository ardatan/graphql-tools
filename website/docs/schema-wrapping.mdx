...
Note that [schema stitching](/docs/stitch-1-combining-schemas) is a _superset_ of the wrapping API. If you want to combine multiple services (with optional transforms) into one combined gateway schema, then you should use the [stitchSchemas](/docs/stitch-1-combining-schemas) method directly and allow it to handle all of the subservice wrappings.
...
- [`RemoveObjectFieldDeprecations`](/docs/api/classes/wrap_src.removeobjectfielddeprecations): accepts a string or regex describing a deprecation to remove from the gateway schema. Fields matching this deprecation will be un-deprecated. Useful for normalizing [computed fields](/docs/stitch-2-type-merging#computed-fields) that are activated by the gateway wrapper.
