...
- `resolvers`: accepts [standard resolvers](/docs/resolvers/) with the addition of specifying a [`selectionSet`](/docs/stitch-4-schema-extensions#selectionset).
- `mergeTypes`: specifies a strategy for [handling duplicated types](/docs/stitch-1-combining-schemas#duplicate-types).
- `typeMergingOptions`: allows customization of [automatic type merging](/docs/stitch-1-combining-schemas#automatic-merge).
- `onTypeConflict`: allows customization of [manual type resolution](/docs/stitch-1-combining-schemas#manual-resolution).

### createMergedTypeResolver

Creates a merged type resolver that may be [wrapped with custom behaviors](/docs/stitch-2-type-merging#wrapped-resolvers).
...
Creates a dynamic `selectionSet` that [forwards gateway arguments](/docs/stitch-4-schema-extensions#via-selectionset) to a resolver selection hint.
