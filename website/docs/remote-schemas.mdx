...
import { observableToAsyncIterable, Executor } from '@graphql-tools/utils'
...
import { fetch } from 'cross-undici-fetch';
import AbortController from 'abort-controller'; // node < v15
    ...
    subscribe: observer => ({
      unsubscribe: subscriptionClient.subscribe(
        ...
        },
        {
          ...
          error: error => observer.next && observer.error(err),,
          complete: () => observer.complete && observer.complete(),
        }
      ),
    }),
...
const httpExecutor: Executor = async ({ document, variables }) => {
  const query = print(document)
...
const wsExecutor: Executor = ({ document, variables }) =>
  observableToAsyncIterable({
    subscribe: observer => ({
      unsubscribe: subscriptionClient.subscribe(
        {
