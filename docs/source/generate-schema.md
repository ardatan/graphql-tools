...
- `typeDefs` is a required argument and should be a GraphQL schema language string, or an array of schema strings, or an object whose values are schema strings, or a function that takes no arguments and returns any of these types. The order of the strings in the array is not important, but it must include a schema definition.
